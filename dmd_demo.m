...
subplot(4,1,3), plot(x,-sech(x).*tanh(x)/max(sech(x).*tanh(x)))
subplot(4,1,4), plot(x,sech(x+3)/max(sech(x+3)))
...
E1(3)=norm(IC(1,:)/max(IC(1,:))-(-sech(x).*tanh(x)/max(sech(x).*tanh(x))));
E2(3)=norm(IC(2,:)/max(IC(2,:))-(sech(x+3)/max(sech(x+3))));
...
% 
% figure(3)
% subplot(2,2,1), plot(diag(Sigma2)/sum(diag(Sigma2)),'ko')
% subplot(2,2,3), plot(x,Phi(:,1),x,Phi(:,2))
%  
% %break
% u=f(1,:).';
